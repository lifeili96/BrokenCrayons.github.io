...
- 根据等式，
- SPSS操作步骤：
  - **Analyze** → **Generalized Linear Models** → **Generalized Linear Models** → 参数设定：「Type of model」-「Binary Response or Events/Trials Data」选择「Binary probit」（对应的是probit函数）或「Binary logit」（对应的是logit函数）
#### 2. 通过变量**yb_L**进行验证
